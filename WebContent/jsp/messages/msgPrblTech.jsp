<%@ include file="/declaration/taglibs.jsp"%>
 
 
<div id="EXC_MESSAGE_INFO_ERREUR_RED">
Erreur lors de la tentative de connexion ...
</div>
  
 
<td  class="LOGIN_BACKG" align="left">
<table width="100%" heigth = "50%" border="0" cellspacing="0" cellpadding="0" class="LOGIN_BACKG" align="center">
  <tr>
<td align="center" Class="CORPS_DONNE"><font color="#000099" size="2" face="Verdana, Arial, Helvetica, sans-serif">
<p><strong>Espaces Sécurisés inaccessible</strong></p>
<p>Bonjour,</p> 
<p>En raison de problèmes techniques, nous ne pouvons donner suite à votre demande de connexion.<br>
  </p>
  <p>Nous vous présentons toutes nos excuses pour la gêne occasionnée et vous invitons à renouveler votre demande ultérieurement.<BR><BR></p>
  
</font>

<p><font size="1"><a href="http://www.mc-alsace.fr/">Retour au site</a></font></p>
</td>
  </tr>
  <tr>
    <td align="center" valign="top" >
    
    
<p>			</p>  
    </td>
  </tr>
    <tr>
    <td  align="center" valign="top"  heigth = "25">
</td>
  </tr>
  
</table>



</td>


