...
StrutsLayout.Datagrid = function Datagrid(in_property, in_styleId, in_styleClass, in_styleClass2, in_allowSelection, in_allowMultipleSelection) {
  ...
  this.removeDatagridRow = removeDatagridRow;
	...
	// Remove a row :
	function removeDatagridRow(index) {
		var datagridName = "strutsLayoutDatagridData['"+this.property+"']";
		
		// Get the datagrid.
		var datagrid = strutsLayoutDatagridData[this.property];
		...
		//
		// And shift back the position
		// (renaming the elements after the deleted one)
		
	
		for (numLigne =index+1; numLigne <= table.rows.length; ++numLigne) {

			for (i in datagrid.columns) {
				var col = datagrid.columns[i];

				toRename = document.getElementsByName( this.property+"."+col.property+"["+numLigne+"]" )[0];
				
				toRename.setAttribute("name" , this.property+"."+col.property+"["+(numLigne-1)+"]");
				
				// if it's a delete button, ever change the link :
				if (col.type == "deleteimg") {
					toRename.setAttribute("href","javascript:"+datagridName+".removeDatagridRow("+(numLigne - 1)+")");
				}
	...
	function addDatagridCell(row, index, property, styleClass, type, values) {
		...
		if (type!="empty") {
			...
			// the type of the tag :INPUT,SELECT or A corresponding to <INPUT ... or <SELECT...
			var inputElementName;
			if (type=="select") inputElementName =  "SELECT";
			else if (type=="deleteimg") inputElementName = "A";
			else inputElementName = "INPUT";
			
			// Create the element of the tag type specified in inputElementName
			var input = document.createElement(inputElementName);
	
			if ( type != "select" && type != "deleteimg" ) {
		  ...
		  if (type=="checkbox") {	   
			...
			}	else if (type=="deleteimg") {
	
					input.setAttribute('href',"javascript:"+datagridName+".removeDatagridRow("+index+")");
	
					var img = document.createElement("IMG");
					img.setAttribute("src", imgsrc + 'delete.gif');
